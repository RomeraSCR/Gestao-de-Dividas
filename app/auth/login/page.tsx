...
export default function LoginPage() {
  ...
  const handleDemo = async () => {
    ...
    setError(null)
    try {
      // Navega para o route handler /demo, que seta cookie e redireciona pro dashboard.
      window.location.href = "/demo"
    } catch {
      setError("Erro ao entrar no modo demonstração. Tente novamente.")
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-gradient-to-br from-blue-50 via-pink-50 to-blue-50 dark:from-slate-950 dark:via-blue-950 dark:to-slate-900 p-4 sm:p-6 md:p-8">
      ...
      <div className="fixed top-4 left-4 z-50">
        <Button
          ...
          size="icon"
          variant="ghost"
          className="rounded-full bg-white/40 dark:bg-slate-900/30 backdrop-blur hover:bg-white/60 dark:hover:bg-slate-900/45 border border-white/30 dark:border-blue-500/25"
          aria-label="Voltar para a Home"
        >
          <Link href="/">
            <ArrowLeft className="h-5 w-5 text-slate-700 dark:text-gray-200" />
          </Link>
        </Button>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md animate-fade-in px-4">
        ...
        >
          <div className="mb-6 space-y-2 text-center">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-gray-100 sm:text-3xl">Bem-vindo</h2>
            <p className="text-sm text-slate-600 dark:text-gray-400">Entre com suas credenciais</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <Button
              type="button"
              variant="outline"
              className="h-11 w-full bg-white/50 dark:bg-slate-900/30 backdrop-blur hover:bg-white/70 dark:hover:bg-slate-900/40 border-white/40 dark:border-blue-500/30"
              disabled={isLoading}
              onClick={handleDemo}
            >
            ...
            <div className="text-center text-sm text-slate-600 dark:text-gray-400">
              Não tem uma conta?{" "}
              <Link href="/auth/cadastro" className="font-semibold text-blue-600 dark:text-blue-400 transition-colors hover:text-pink-600 dark:hover:text-blue-300">
                Cadastre-se
              </Link>
            </div>
          </form>
